...
import {
    ChatService, IChatChannel, IChatParticipants, ITextMessage, IUser, OnlineUser,
    PagedList
...
export class ChatPage implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(
    ...
    if (!this.hubConnected) {
      ...
      this.logger.log('reserve id', this.reserveId);
  ...
  setChatSubscription() {
    ...
    this.chatSubscription.add(this.chatSvc.connection$.subscribe((connected: boolean) => {
      this.hubConnected = connected;
    }));

    this.chatSubscription.add(this.chatSvc.newOnlineUser$.subscribe((res) => {
      this.logger.log(res);
    }));

    this.chatSubscription.add(this.chatSvc.join$.subscribe((channelId) => {
      this.channelId = channelId;
    }));

    this.chatSubscription.add(this.chatSvc.connectionError$.subscribe((reason: any) => {
      this.hubConnected = false;
      this.logger.error('connection error', reason);
    }));

    this.chatSubscription.add(this.chatSvc.createdChannel$.subscribe((channel: IChatChannel) => {
      this.logger.log('created channel', channel);
    }));

    this.chatSubscription.add(this.chatSvc.getChannelList$.subscribe(
      (channelList: PagedList<IChatChannel>) => {
        this.logger.log('get channel list', channelList);
      }
    ));

    this.chatSubscription.add(this.chatSvc.recvMessage$.subscribe((message: ITextMessage) => {
    ...
    this.chatSubscription.add(this.chatSvc.newOnlineUser$.subscribe((user: OnlineUser) => {
      const newParticipants: IChatParticipants = {};
      this.participants.push(newParticipants);
    }));

    this.chatSubscription.add(this.chatSvc.leave$.subscribe((leave) => {}));

    this.chatSubscription.add(this.chatSvc.getParticipants$.subscribe((participants) => {}));
  }
  ...
  join() {
    this.joinAsync(this.reserveId);
  }

  async joinAsync(reserveId: number) {
    if (!this.hubConnected) {
      this.authSvc.extendToken().subscribe(async (isRefresh) => {
        if (isRefresh) {
          await this.chatSvc.startAsync().then(() => {
              this.logger.log('실시간 서버에 연결하였습니다.');
              this.chatSvc.joinAsync(reserveId)
                .then(() => {
